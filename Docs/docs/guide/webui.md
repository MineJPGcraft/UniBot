...
| **::fluent-color:checklist-24:: 任务中心** | 右上角入口：查看依赖同步、扩展安装、插件安装、Studio 启动等后台任务的进度与日志，支持取消与重试 |
...
**右上角**的任务列表图标（角标显示进行中的任务数）以抽屉形式列出全部后台任务：

- 覆盖**依赖同步、扩展市场安装/卸载、扩展热重载、Studio 下载与启动、插件市场安装/升级/卸载、版本更新**。
- 每个任务显示状态（排队中 / 执行中 / 已完成 / 失败 / 已取消）、阶段说明、创建时间与耗时，运行中显示进度条。
- 展开任务可查看实时日志（通过 WebSocket 推送），并在需要时**取消**或**重试**任务。
- 抽屉内可直接**手动触发依赖同步**（写入一律通过 `uv add` / `uv remove`，无需手工编辑 `pyproject.toml`）。
