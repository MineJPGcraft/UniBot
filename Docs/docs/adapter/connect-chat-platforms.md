...
::: warning 务必在群设置中开启消息权限（重要）
QQ 官方机器人默认**无法在群聊中主动发送消息**，也可能收不到群消息。请进入 **QQ 群 → 群设置 → 群机器人**，找到你的机器人并确认已开启：

- **允许机器人主动发送消息**（否则播报、进群欢迎、上下线通知等主动推送不会发出）
- **允许机器人接收消息**（否则群内 @ 机器人的消息不会下发到机器人）

若未开启上述权限，机器人可能表现为「指令不响应」或「不主动发消息」，这是平台侧的权限限制，与 UniBot 配置无关。
:::
